library(stringr) # [changed] added this import to fix "str_pad" function missing
library(rjson) # [changed] added this import for saving as json instead of RDS
...
for (i in 1:nsim){
  ...
  # for saving
  Rth.accept <- rep(0, mcmc$nmc) # [changed] moved into own line to fix array ref problem

  theta.accept <- rep(0, mcmc$nmc) # [changed] moved into own line to fix array ref problem
  ...
  filename_i <- paste0("MH_n100_s", si_num, ".json") # [changed] file extension from RDS to json

  write(toJSON(results_i), file.path(wd_rproj, "_MCMC_output", "_simstudy_results", filename_i)) # [changed] writing data to json instead of rds
  
  
  # mean(muRth_MH[(mcmc$burnin + 1):mcmc$nmc]) # [changed] disabled line since it does not do anything # TODO: fix problem with this line
  # mean(sigmaRth_MH[(mcmc$burnin + 1):mcmc$nmc]) # [changed] disabled line since it does not do anything # TODO: fix problem with this line
