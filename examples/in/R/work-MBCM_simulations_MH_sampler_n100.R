...
for (i in 1:nsim){
  ...
  mcmc <- list()
  # defaultMCMC <- list(M = 100, burnin = 20, thin = 1)
  ...
  # for saving
  # Rth.accept <- theta.accept <- rep(0, mcmc$nmc) # Moved into two seperate lines in order to fix array reference issue
  Rth.accept <- rep(0, mcmc$nmc) # important to write this as two lines
  theta.accept <- rep(0, mcmc$nmc) # important to write this as two lines
  ...
  # cat("\nMCMC info: \n", "starttime:  ", starttime, "\n", "durT:  ", durT, "\n", "durM:  ", durM, "\n", "time per iteration:  ", durT/mcmc$nmc, "\n",  "finish:", finish, "\n\n")
